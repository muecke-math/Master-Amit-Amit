...
def train(
    ...
    if cfg.get("tensorboard"):
        # Create TensorBoard callback using the path from the config file
        tensorboard_callback = tf.keras.callbacks.TensorBoard(
            log_dir=cfg.tensorboard.log_dir,  # Use the path from config
            histogram_freq=cfg.tensorboard.histogram_freq,
            write_graph=cfg.tensorboard.write_graph,
            update_freq=cfg.tensorboard.update_freq
        )

        # Ensure the log directory exists
        #os.makedirs(cfg.tensorboard.log_dir, exist_ok=True)
    ...
    if cfg.get("test"):
        ...
        # trainer = hydra.utils.instantiate(cfg.trainer) # Remove this later
