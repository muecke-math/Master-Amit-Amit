...
class FCN(object):
    def __init__(self, layers, lb, ub, output_names, discrete: bool = False, activation="tanh", dtype='float32') -> None:
    ...
    def initalize_net(self, layers):
        ...
        z = self.create_layer(inputs, layers[1], initializer, self.activation, self.dtype)
        
        for i in range(1, len(layers) - 2):
            z = self.create_layer(z, layers[i + 1], initializer, self.activation, self.dtype)
...
class NetHFM(object):
    ...
    def __init__(self, mean, std, layers: List, output_names: List, activation=lambda x: x * tf.sigmoid(x)):
        super().__init__()
