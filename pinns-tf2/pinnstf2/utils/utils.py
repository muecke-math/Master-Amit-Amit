...
def get_metric_value(metric_dict: Dict[str, Any], metric_names: list) -> float:
    ...
    for type_metric, list_metrics in metric_names.items():
        # Decide prefix based on the type_metric
        if type_metric == "extra_variables":
            prefix = ""
        elif type_metric == "error":
            # This is your "val" block
            prefix = "val/error_"
        elif type_metric == "test":
        ...
        for metric_name in list_metrics:
            ...
            if full_key not in metric_dict:
                log.info(
                    f"Metric value not found! <metric_name={full_key}>"
                )
            else:
                metric_value = metric_dict[full_key]
                # if it's a tensor, you might do .item() or .numpy()
                if hasattr(metric_value, "numpy"):
                    metric_value = metric_value.numpy()
                elif hasattr(metric_value, "item"):
                    metric_value = metric_value.item()

                results[full_key] = metric_value
                log.info(f"Retrieved metric value! <{full_key}={metric_value}>")
