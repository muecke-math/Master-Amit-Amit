...
def mae(loss: tf.Tensor,
        preds: Dict[str, tf.Tensor],
        target: Optional[Dict[str, tf.Tensor]] = None,
        keys: Optional[List[str]] = None,
        mid: Optional[int] = None) -> tf.Tensor:
    """Calculate the mean absolute error (MAE) loss for given predictions and optional targets."""
    ...
    for key in keys:
        if target is None and mid is None:
            loss = loss + tf.reduce_mean(tf.abs(preds[key]))
        elif target is None and mid is not None:
            loss = loss + tf.reduce_mean(tf.abs(preds[key][:mid] - preds[key][mid:]))
        elif target is not None:
            loss = loss + tf.reduce_mean(tf.abs(preds[key] - target[key]))

    return loss

def huber_loss(loss: tf.Tensor,
               preds: Dict[str, tf.Tensor],
               target: Optional[Dict[str, tf.Tensor]] = None,
               keys: Optional[List[str]] = None,
               mid: Optional[int] = None,
               delta: float = 1.0) -> tf.Tensor:
    """Calculate the Huber loss for given predictions and optional targets."""
    ...
    for key in keys:
        if target is None and mid is None:
            loss = loss + huber(tf.zeros_like(preds[key]), preds[key])
        elif target is None and mid is not None:
            loss = loss + huber(preds[key][:mid], preds[key][mid:])
        elif target is not None:
            loss = loss + huber(target[key], preds[key])
