...
cwd = os.getcwd()
layer_sizes = [2, 50, 50, 50, 50, 1] # no added extra layer
...
def neural_net(layer_sizes):
    ...
    for width in layer_sizes[1:-1]:
        model.add(layers.Dense(
            width, activation=tf.nn.tanh, # tf.nn.swish, tf.nn.softplus, tf.nn.sigmoid
...
def loss(x_f, y_f,
    ...
    # # modified: use sum of squared errors for boundary predictions (SSE)
    # mse_b_u = (tf.reduce_sum(tf.square(u_lb_pred)) +
    #                  tf.reduce_sum(tf.square(u_ub_pred)) +
    #                  tf.reduce_sum(tf.square(u_rb_pred)) +
    #                  tf.reduce_sum(tf.square(u_lftb_pred)))

    # # modified: use sum of squared errors for the PDE residual loss (SSE)
    # mse_f_u = tf.reduce_sum(tf.square(col_weights * f_u_pred))


    # ## Adding loss (mae, huber, sse) ##########
    # # added: define Huber loss function with a chosen delta value (can be tuned)
    # loss_fn = tf.keras.losses.Huber(delta=1.0)
    # # loss_fn = tf.keras.losses.MeanAbsoluteError()

    # # modified: replace MSE on boundary predictions with Huber loss
    # mse_b_u = (loss_fn(tf.zeros_like(u_lb_pred), u_lb_pred) +
    #                  loss_fn(tf.zeros_like(u_ub_pred), u_ub_pred) +
    #                  loss_fn(tf.zeros_like(u_rb_pred), u_rb_pred) +
    #                  loss_fn(tf.zeros_like(u_lftb_pred), u_lftb_pred))

    # # modified: replace MSE on the PDE residual with Huber loss
    # mse_f_u = loss_fn(tf.zeros_like(f_u_pred), col_weights * f_u_pred)
...
def fit(x_f, t_f, x_lb, y_lb, x_ub, y_ub, x_rb, y_rb, x_lftb, y_lftb, col_weights, tf_iter, newton_iter):
    ...
    # Nadam Optimizer
    # tf_optimizer = tf.keras.optimizers.Nadam(learning_rate=0.001, beta_1=0.99)
    # tf_optimizer_coll = tf.keras.optimizers.Nadam(learning_rate=0.001, beta_1=0.9)
    # RMSprop optimizer 
    # tf_optimizer = tf.keras.optimizers.RMSprop(learning_rate=0.001)  
    # tf_optimizer_coll = tf.keras.optimizers.RMSprop(learning_rate=0.001)
    # Adagrad optimizer
    # tf_optimizer = tf.keras.optimizers.Adagrad(learning_rate=0.001)
    # tf_optimizer_coll = tf.keras.optimizers.Adagrad(learning_rate=0.001) 
...
def test_error_u():
    # Define a test grid (using a coarser grid for speed)
    ...
    # Get the model prediction using your predict function.
    u_pred_test, _ = predict(X_star_test)
    
    # Compute the relative error.
    error_u = np.linalg.norm(u_star_test - u_pred_test, 2) / np.linalg.norm(u_star_test, 2)
...
plt.savefig(os.path.join(cwd, 'Helmholtz/Extra_layer50/U_pred_plot.png'))
# plt.show()
...
plt.savefig(os.path.join(cwd, 'Helmholtz/Extra_layer50/FU_pred_plot.png'))
# plt.show()

# print collocation point weights
# plt.scatter(x_f, y_f, c = col_weights.numpy(), s = col_weights.numpy()/100)
# plt.savefig(os.path.join(cwd, 'Helmholtz/he_collocation_weights.png'))
# # plt.show()
...
plot_helmholtz_figure(U_pred=U_pred,
                      ...
                      lb=[-1.0, -1.0],
                      ub=[1.0, 1.0],
                      save_dir=os.path.join(cwd, 'Helmholtz/Extra_layer50/baseline.png'))
